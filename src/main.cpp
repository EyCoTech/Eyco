...
unsigned int nStakeMinAge =  6 * 60 * 60; // 1 hours
unsigned int nStakeMaxAge = 31 * 24 * 60 * 60; // 21 Days
unsigned int nModifierInterval = 13 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 65; // 66 in total
...
const string strMessageMagic = "Eyco Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (pindexBest->nHeight < 333)
    {
        nSubsidy = 12012012 * COIN;
    }
...
static int64_t nTargetTimespan = 13 * 60;  // 14 mins
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fValidateSig)
{
    ...
    // fMiner is true when called from the internal eyco miner
