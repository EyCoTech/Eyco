    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Eyco and make it unusable. To
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeEycoAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fEycoSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f EYCO CENT - sessionTotalValue %d\n", fEycoSubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        //try to sync the masternode list and payment list every 5 seconds from at least 2 nodes
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeEycoAmount = (rand() % nLeftToAnon)+3;
                } else {
