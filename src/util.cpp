...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Eyco
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Eyco
    // Mac: ~/Library/Application Support/Eyco
    // Unix: ~/.Eyco
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Eyco";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "eyco.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No eyco.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override eyco.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "eycod.pid"));
...
// Format the subversion field according to BIP 14 spec (https://en.eyco.it/wiki/BIP_0014)
