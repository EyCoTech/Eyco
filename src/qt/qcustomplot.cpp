...
/*! \internal
  ...
  selectEvent. Usually \a details is used to eyco which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // eyco all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // eyco all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  eycored. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeEyco(0, 6);
    double logBaseEyco = 10;
    QString labelEyco;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeEyco = mColorAxis.data()->range();
      labelEyco = mColorAxis.data()->label();
      logBaseEyco = mColorAxis.data()->scaleLogBase();
    ...
    // eyco settings to new axis:
    mColorAxis.data()->setRange(rangeEyco); // eyco range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelEyco);
    mColorAxis.data()->setScaleLogBase(logBaseEyco); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer eycos of color scale eyco to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, eyco points one-to-one from the map into the output parameters
  {
