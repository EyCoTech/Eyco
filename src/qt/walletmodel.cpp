...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CEycoAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
...

    {
		...
		else
		{
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CEycoAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
	...
	std::string strFailReason;
        ...
        /*if(recipients[0].useInstantX && total > GetSporkValue(SPORK_2_MAX_VALUE)*COIN){
            emit message(tr("Send Coins"), tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to %n EYCO.", "", GetSporkValue(SPORK_2_MAX_VALUE)),true,
...
static void NotifyAddressBookChanged(WalletModel *walletmodel, CWallet *wallet, const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
        LogPrintf("NotifyAddressBookChanged %s %s isMine=%i status=%i\n", CEycoAddress(address).ToString().c_str(), label.c_str(), isMine, status);
        QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
                                  Q_ARG(QString, QString::fromStdString(CEycoAddress(address).ToString())),
                                  Q_ARG(QString, QString::fromStdString(label)),
                                  Q_ARG(bool, isMine),
                                  Q_ARG(int, status));
    }
...
{
    // Eyco context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CEycoAddress(address).ToString().c_str()].push_back(out);
    }
}
