/*
 * Qt5 bitoin/eyco GUI.
 * The Eyco Developer 2018
 * W.J. van der Laan 2011-2012
 * The Eyco Developers 2011-2012
 */
...
EycoGUI::EycoGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
void EycoGUI::createActions()
{
    ...
    aboutAction = new QAction(tr("&About Eyco"), this);
    aboutAction->setToolTip(tr("Show information about Eyco"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Eyco"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/eyco"), tr("&Show / Hide"), this);
...
void EycoGUI::setClientModel(ClientModel *clientModel)
{
    if(!fOnlyTor)
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/eyco_testnet"));
            setWindowIcon(QIcon(":icons/eyco_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/eyco_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Eyco client") + QString(" ") + tr("[testnet]"));
...
void EycoGUI::setWalletModel(WalletModel *walletModel)
{
...
void EycoGUI::setMessageModel(MessageModel *messageModel)
{
...
void EycoGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Eyco client"));
...
#ifndef Q_OS_MAC
void EycoGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EycoGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Eyco network", "", count));
}

void EycoGUI::setNumBlocks(int count)
{
    QString tooltip;
...
void EycoGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Eyco") + " - ";
...
void EycoGUI::changeEvent(QEvent *e)
{
...
void EycoGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void EycoGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(EycoUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void EycoGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(EycoUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void EycoGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void EycoGUI::gotoMasternodeManagerPage()
{
...
void EycoGUI::gotoAddressBookPage()
{
...
void EycoGUI::gotoReceiveCoinsPage()
{
...
void EycoGUI::gotoSendCoinsPage()
{
...
void EycoGUI::gotoSignMessageTab(QString addr)
{
...
void EycoGUI::gotoVerifyMessageTab(QString addr)
{
...
void EycoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void EycoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Eyco address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void EycoGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void EycoGUI::setEncryptionStatus(int status)
{
...
void EycoGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void EycoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void EycoGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void EycoGUI::updateStakingIcon()
{
    updateWeight();
