...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CEycoAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            ...
            if(CEycoAddress(rec->address.toStdString()) == CEycoAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CEycoAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CEycoAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CEycoAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
{
    // Update address book model from Eyco core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CEycoAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CEycoAddress(strAddress).Get(), strLabel);
            }
        }
    }
    else if(type == Receive)
    {
        ...
        } else
        {
            ...
            strAddress = CEycoAddress(newKey.GetID()).ToString();
...
{
    {
        ...
        {
            CEycoAddress address_parsed(sAddr);
