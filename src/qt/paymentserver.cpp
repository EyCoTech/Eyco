...
const int EYCO_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString EYCO_IPC_PREFIX("eyco:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(EYCO_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(EYCO_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(EYCO_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click eyco: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start eyco: click-to-pay handler");
    else
...
{
    // clicking on eyco: URLs creates FileOpen events on the Mac:
