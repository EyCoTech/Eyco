    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Eyco address (e.g. CgzSt8jt5r4tQXKg8G4qrGEd6GSFXY7AA1)"));
#endif
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::eycoAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(EycoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(EycoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    // URI has to be valid
    if (GUIUtil::parseEycoURI(uri, &rv))
    {
        CEycoAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(EycoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CEycoAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        CoinControlDialog::coinControl->destChange = CEycoAddress(text.toStdString()).Get();

        // label for the change address
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:black;}");
        if (text.isEmpty())
            ui->labelCoinControlChangeLabel->setText("");
        else if (!CEycoAddress(text.toStdString()).IsValid())
        {
            ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:red;}");
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Eyco address"));
        }
        else
        {
            ...
            else
            {
                CPubKey pubkey;
                CKeyID keyid;
                CEycoAddress(text.toStdString()).GetKeyID(keyid);   
