...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #eycocoinTEST\r");
            Send(hSocket, "WHO #eycocoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #eycocoin\n");
            // randomly join #eycocoin00-#eycocoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #eycocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #eycocoin%02d\r", channel_number).c_str());
        };
